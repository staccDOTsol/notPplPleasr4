...
import { upload } from './helpers/upload/arweave'
import {
  Commitment,
  Connection,
  Context,
...
programCommand('burn_crank')
  ...
  .action(async (options) => {
    ...
    const wallet = loadWalletKey(options.keypair);
    const anchorProgram = await loadFireballProgram(wallet, options.env);
    const connection = new Connection("https://solana-mainnet.g.alchemy.com/v2/WM_Gl7ktiws7icLQVxLP5iVHNQTv8RNk")
...
export async function arweaveUpload(
  ...
  const result = await upload(data, manifest, 0);
console.log(result)
