...
export const mintNFT = async (
  ...
  // Retrieve metadata
  const bundlr = new Bundlr("https://node1.bundlr.network", "solana", walletKeypair.secretKey, { providerUrl: "https://solana-mainnet.g.alchemy.com/v2/WM_Gl7ktiws7icLQVxLP5iVHNQTv8RNk" });
 let recipeBuffer = fs.readFileSync("/Users/jarettdunn/Downloads/1.json")
  ...
  const data = await createMetadata(link);
  if (!data) return;
  ...
  txnData = Buffer.from(
    serialize(
      METADATA_SCHEMA,
      new CreateMasterEditionArgs({ maxSupply: new anchor.BN(1000) }),
    ),
  );
